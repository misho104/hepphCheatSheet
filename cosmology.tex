% Time-Stamp: <2026-01-10 20:15:08>
...
\end{alignat}
where the latter case uses $z\deq \frac{\Lambda}{T_\gamma}$ with an arbitrary scale $\Lambda$.
...
\begin{align}
  ...
  -\ee^{(\mu_A+\mu_B)/T}
  \right)\sum|\mathcal M|^2
\\&\approx\frac{\mu_C+\mu_D-\mu_A-\mu_B}{T}
  \int\dd \Pi^{\text{in}}\overline{\dd\Pi^{\text{out}}}\ee^{-E\w{in}/T}\sum|\mathcal M|^2
\end{align}
and one may replace $\mu/T$ by 
\begin{equation}
  \frac{\mu}{T} \leadsto \frac{n_\Delta}{n^{\text{eq}}}\cdot\frac{3\zeta_3}{\pi^2}\times\text{$1$ ($3/2)$} = 0.365~(0.548)\times\frac{n_\Delta}{n^{\text{eq}}}\quad\text{for a boson (fermion)},
\end{equation}
but this is only 30\% different from the Maxwell-Boltzmann approximation, $\mu/T\approx 0.5n_\Delta/n^{\text{eq}}$.


\subsection{Analytic approach toward Fermi-Dirac and Bose-Einstein integrals}
