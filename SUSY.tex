...
%%% Time-Stamp: <2011-03-14 21:00:45 misho>
...
\paragraph{$F$-term Scalar Potential with $R$-parity}
\begin{align}
 V^F_{\Hu}&=
 |\mu|^2\left(|\Hd|^2+|\Hu|^2\right)
\notag\\&\qquad
 - \left(\mu^*\Hd^*Qy^u\bU + \mu^*\Hu^*Qy^d\bD + \mu^*\Hu^*Ly^e\bE + \Hc\right)
\notag\\&\qquad
 + |\Hu|^2|y^u_{ij}\bU_j|^2 +|\Hd|^2|y^d_{ij}\bD_j|^2 +|\Hd|^2(\bE^*y^{e\dagger}y^e\bE)
\\&\qquad
 + \sum_a\Bigl(|Q^ay^u\bU|^2 + |Q^ay^d\bD|^2+|L^ay^e\bE|^2\Bigr)
\notag\\&\qquad
+\Bigl[
(Qy^d\bD)^*(Ly^e\bE) - y^{u*}_{ij}y^d_{ik} \Hu^*\Hd\bU_j^*\bD_k +\Hc
\Bigr]
\notag\\&\qquad
+\epsilon^{ab}\epsilon^{cd}\left[
 y^{u*}_{ji}y^u_{ki}H\s u^{a*}H\s u^cQ_j^{b*}Q_k^d
+y^{d*}_{ji}y^d_{li}H\s d^{a*}H\s d^cQ_j^{b*}Q_l^d
+y^{e*}_{ji}y^e_{li}H\s d^{a*}H\s d^cL^{*b}_jL_l^d
+\Hc\right]
\notag
\end{align}
