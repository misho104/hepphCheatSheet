...
%%% Time-Stamp: <2010-11-14 19:46:06 misho>
%%% 文字コードはShift-JISですよね
...
\begin{align}
  ...
  F_{\mu\nu} & \mapsto VF_{\mu\nu}V^{-1};&
  \phi &\mapsto \tilde V\phi,
\end{align}
where $\tilde V$ is in arbitrary representation. The covariant derivative can be expressed as
\begin{equation}
 \Dm = \Pm - X_\mu;\where \Pm V=[X_\mu,V].
\end{equation}

If the gauge group $G$ is {\bf compact}, it has a finite-dimensional
unitary representation. The generators $t_a$ can be taken as Hermitian, and $V = \exp\left[\ii \alpha^a t^a\right]$ for $\alpha^a\in\Real$;
\begin{align}
 [t^a,t^b] &= \ii f\T^a^b_c t^c\quad(f\in\Real)&
 0 &=f\T^D_a_b f\T^E_D_c + f\T^D_c_a f\T^E_D_b  + f\T^D_b_c f\T^E_D_a
\end{align}
Here we can normalize the generators as $\Tr T^aT^b=\frac12\delta^{ab}$, and the structure constant $f^{abc}$ would be totally antisymmetric.
\begin{align}
