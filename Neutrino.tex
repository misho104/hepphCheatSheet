...
If we set $M_N =0$ in the Type-I see-saw ($n\w{RHN}=3$), AT-diagonalization results in 
\begin{align}
...
\end{align}
and we have three pairs of degenerate Weyl fermions, i.e., three Dirac neutrinos, which is equivalent to the Dirac neutrino model.

\subsection{Weinberg Operator}
The neutrino masses are often described by the Weinberg operator~\cite{Weinberg:1979sa}, which is obtained by integrating out the right-handed neutrinos. It is given by, usually with the convention $C_{ij}=C_{ji}$ because of $(L_i H)(L_j H)=(L_j H)(L_i H)$,
\begin{equation}
  \mathcal L\ni \frac{C_{ij}}{2}(L_i H)(L_j H)+\text{H.c.}
  \equiv
  \frac{C_{ij}}{2}\epsilon^{ab}\epsilon^{cd}(L_i^a H^b)^\alpha(L_j^c H^d)_\alpha+\text{H.c.}
  \leadsto
  \frac{C_{ij}}{2}\mean{H^0}^2(\nu_i\nu_j)+\text{H.c.}
\end{equation}
The (Majorana) mass matrix is thus given by $C_{ij}\mean{H^0}^2=C_{ij}v^2/2$ with $v=246\GeV$.
The overall sign is unphysical here but may appear in the PMNS matrix.
...
As experiments have not found any deviation from $[U]\w{upper left}$ being unitary, we express the mixing with the PMNS matrix
 \GRAY{(Pontecorvo--\JA{牧}--\JA{中川}--\JA{坂田})}
parameterized by
\begin{equation}
\begin{split}
   U\w{PMNS}&=
