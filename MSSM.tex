...
In super-CKM basis, the fermion-related interaction terms are
\begin{equation}
  \begin{split}
...
+g_Z Z_\mu\Bigl[
  ...
  \\&
  + \left[\cw^2 V_{j1} V_{i1}^*+{V_{j2} V_{i2}^* (\cw^2-\sw^2)}/{2}\right] \bcharP[j]\bsigma^{\mu}\charP[i]
  + \left[{U_{j2} U_{i2}^* (\sw^2-\cw^2)}/{2}-\cw^2 U_{j1} U_{i1}^*\right]{\bcharM[j]\bsigma^{\mu }\charM[i]}
\Bigr]
...
\end{align*}
we have, noting that squarks and sleptons are in super-CKM basis and not in mass eigenstates: (next page)
\clearpage
\begin{align}
&  \mathcal L\w{fermions} + \mathcal L\w{SFG}
\notag\\&=
...
\begin{minipage}[t]{0.5\textwidth}
  The SLHA convention \cite{SLHA} is different from our notation.
  the reinterpretation rules for the parameters are given in the right table  (\textbf{\C{magenta color}} for objects in other conventions), while
  \[
    \mu, b, m^1_{Q, L, \Hu, \Hd}, \text{RPV-trilinears ($\lambda$s and $T$s)}
  \]
   are in common.
\end{minipage}
\begin{minipage}[t]{0.48\textwidth}\hfill
