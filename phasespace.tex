...
\begin{equation}
 \overline{\dd\Pi^{n}} := \dd\Pi_1\dd\Pi_2\cdots\dd\Pi_n\,(2\pi)^4\ddelta[4]\left(P_0-\sum p_n\right);
\quad
...
= \int\frac{\dd p_0\dd^3\vc p}{(2\pi)^4}(2\pi)\ddelta\left(p_0^2-\|\vc p\|^2-m^2\right)\Theta(p_0)
\end{equation*}
K\"all\'en function:
\begin{align*}
&\mathop{\lambda}(x,y,z)
...
As
$\braket{\text{out}|\text{in}}=(2\pi)^4\ddelta[4](p\w i-p\w f)\ii\mathcal M$
(for $\text{in}\neq\text{out}$) and $\braket{\vc p|\vc p}=2E_{\vc p}(2\pi)^3\ddelta[3](\vc 0)=2E_\vc {p}V$ for one-particle state,
\begin{equation}
...
\frac{VT}{(2\pi^4)}\ddelta[4](p\w i-p\w f)
